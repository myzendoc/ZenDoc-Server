...
udpsrc port={audio_remote_rtp_port} caps="{audio_caps}" ! rtpbin.recv_rtp_sink_0 rtpbin. ! rtpopusdepay ! opusdec plc=false ! queue max-size-buffers=2 max-size-time=0 max-size-bytes=0 leaky=downstream ! audioconvert ! audioresample ! audio/x-raw, format=S16LE, rate=16000, channels=1 ! appsink name=audio_sink emit-signals=true sync=false max-buffers=1 drop=true
            ...
            with deepgram.listen.v1.connect(
                ...
                def sender_worker():
                    while True:
                        ...
                        try:
                            data = audio_queue.get(block=True, timeout=0.1)
